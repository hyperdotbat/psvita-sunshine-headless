...
`sudo cp ./etc/systemd/system/xorg-headless.service /etc/systemd/system/`  
`sudo chown root:root /etc/systemd/system/xorg-headless.service`  
`sudo systemctl enable xorg-headless`

Copy over the EDID file:  
`sudo cp ./usr/lib/firmware/edid/720p.bin /usr/lib/firmware/edid/`  
...
`cp ./.config/sunshine/apps.json $HOME/.config/sunshine/` 
...
or you can use Tailscale or other things, there are more posts and guides talking about this entire topic.
...
If youre gonna have any trouble later go through the Advanced tab of the config, try different combinations; for Streaming method if KMS is not working then switch to X11, for Encoder if VA-API is glitching like mentioned below use Software.  
